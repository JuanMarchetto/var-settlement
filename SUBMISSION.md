...
- **Two open soundness items, flagged not hidden** (full self-audit in `docs/AUDIT.md`, trust surface in `docs/TRUST_SURFACE.md`):
  - **Match status is a resolver input, not oracle-authenticated.** `validate_stat` proves the goal counts against the Merkle root; the match-status code is supplied by the caller. The proven rulebook fail-closes every non-`Completed` status to `Refund` (INV-1), so the downside is bounded to a refund — never a fabricated decisive result — but binding status to an authenticated feed field is a gap to close before mainnet.
  - **`fixture_id` not asserted on-chain.** `resolve` binds each witness to the market's stat-key/period but does not yet assert `fixture_id` equality directly; today the resolver's witness *selection* is trusted to supply a same-fixture pair (upstream stat-key scoping makes a mixed pair hard to pass, but the program doesn't reject it). A soundness gap to tighten, not cosmetic polish.
