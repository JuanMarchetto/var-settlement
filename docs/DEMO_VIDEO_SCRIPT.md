...
- **Pre-warm before rolling (these are slow and the live API token is short-lived):**
  ...
  2. **Tx LINE settle:** run `bun run txline-activate.ts` (fresh live token), then `bun run txline-settle.ts 18192996 770` **before** recording. Tab B on screen is that real run's captured output (caption: `real run — tx signatures on-chain`). The canonical settled market it produces is already on devnet: market PDA `GaiXEuSBb3spjoptxHCoyScycN4sCy164jCF3jT9v8T3`, resolve tx `3uSf…` — the video anchors every Explorer link and the reverify to **that** market so everything a judge clicks is consistent.
...
bun run reverify.ts GaiXEuSBb3spjoptxHCoyScycN4sCy164jCF3jT9v8T3
```
...
- Canonical settled **market PDA** (fixture 18192996 — what `reverify` targets): `GaiXEuSBb3spjoptxHCoyScycN4sCy164jCF3jT9v8T3`
...
- `attest_home` (home goals authenticated): `xuUExMAXeovpdcwMXegQMcpd49mampkqxWNdQze7Lf7ZzuKV1GbiyGppCSA1YbEXtD45byjF6ZS8zTAchba5MbJ`
  https://explorer.solana.com/tx/xuUExMAXeovpdcwMXegQMcpd49mampkqxWNdQze7Lf7ZzuKV1GbiyGppCSA1YbEXtD45byjF6ZS8zTAchba5MbJ?cluster=devnet
- `resolve` (MONEY SHOT — away goals authenticated, rulebook resolves):
  `3uSfesCgBNN8qAjM2CwLjtCVPquaE3cUk2qkZQc7ndnKFbkStaqs26HNyZtPMFa22ZFj1ucuVSda1TNAeuwHxGYD`
  https://explorer.solana.com/tx/3uSfesCgBNN8qAjM2CwLjtCVPquaE3cUk2qkZQc7ndnKFbkStaqs26HNyZtPMFa22ZFj1ucuVSda1TNAeuwHxGYD?cluster=devnet
- Program page: https://explorer.solana.com/address/AepSNpDzMUdBgjxA9irxxL7NTQHxXtDVq6rnqq17Lxk?cluster=devnet
- Market page (the one `reverify` re-derives): https://explorer.solana.com/address/GaiXEuSBb3spjoptxHCoyScycN4sCy164jCF3jT9v8T3?cluster=devnet
...
- **Stranger `reverify` is the safest live moment even if the feed is down** — it re-derives the already-settled market `GaiXEuSBb3spjoptxHCoyScycN4sCy164jCF3jT9v8T3` from an on-chain receipt, independent of Tx LINE availability. Keep it in every cut.
